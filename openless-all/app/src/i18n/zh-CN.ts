...
export const zhCN = {
  ...
  settings: {
    ...
    sync: {
      title: '同步',
      desc: '登录后可把风格包、词汇表和历史等数据同步到官方云端账号。',
      signedIn: '已登录',
      signedOut: '未登录',
      serverUrlLabel: '自定义同步服务器',
      serverUrlDesc: '默认使用官方同步服务。这个地址仅用于开发或测试，私有化部署功能后续开放。',
      deviceNameLabel: '设备名',
      deviceNameDesc: '用于在账号中区分这台设备。',
      saveSettings: '保存',
      saved: '同步设置已保存。',
      accountLabel: '账号',
      accountDesc: '使用邮箱验证码登录；新邮箱会自动创建账号。登录 token 只保存在本机系统凭据库。',
      codePlaceholder: '验证码',
      sendCode: '发送验证码',
      sendingCode: '发送中…',
      login: '登录',
      ...
      loginSuccess: '已登录 {{email}}。',
      logoutSuccess: '已退出同步账号。',
      statusTitle: '同步状态',
      statusDesc: '手动同步只拉取云端变更；本地变更入队与自动推送在后续步骤接入。',
      lastSyncLabel: '最近同步',
      lastSyncDesc: '最近一次 pull / push 成功时间。',
      never: '从未同步',
      pendingLabel: '待同步队列',
      pendingDesc: '离线或失败时保留的本地变更数量。',
      pendingCount: '{{count}} 条',
      manualSyncLabel: '手动同步',
      manualSyncDesc: '从云端拉取最新数据。未登录时不可用。',
      manualSync: '立即同步',
      syncing: '同步中…',
      syncSuccess: '同步完成，当前 cursor：{{cursor}}。',
      clearCloudLabel: '清空云端数据',
      clearCloudDesc: '只清空当前账号的云端同步数据，不删除本机数据。',
      clearCloud: '清空云端',
      clearing: '清空中…',
      clearConfirmFirst: '确定清空当前账号的云端同步数据？本机数据不会被删除。',
      clearConfirmSecond: '再次确认：云端数据清空后不可恢复。继续吗？',
      clearSuccess: '云端同步数据已清空。',
      advancedTitle: '高级设置',
      advancedDesc: '一般不需要修改。当前版本先使用官方同步服务跑通流程。',
      migrationLabel: '数据迁移',
      migrationDesc: '用于未来在官方服务和其他服务器之间迁移同步数据。',
      migrationExport: '导出',
      migrationReupload: '重新上传',
      migrationImport: '导入',
      migrationComingSoon: '迁移功能后续开放，当前按钮不可用。',
    },
    shortcuts: {
      title: '快捷键速查',
