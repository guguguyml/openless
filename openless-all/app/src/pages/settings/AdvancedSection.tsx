...
export function AdvancedSection() {
    ...
    const activeAsrProvider = (prefs?.activeAsrProvider ??
        "volcengine") as AsrPresetId
    ...
    const pendingNameKey =
        pendingTarget === "local-qwen3"
            ? "asrLocalQwen3"
            : pendingTarget === "foundry-local-whisper"
              ? "asrFoundryLocalWhisper"
              : pendingTarget === "sherpa-onnx-local"
                ? "asrSherpaOnnxLocal"
                : null
        ...
        <>
                    ...
                    >
                        ...
                        >
                            {t("settings.advanced.confirmEnableLocalBody", {
                                target: t(
                                    `settings.providers.presets.${pendingNameKey}`,
                                ),
                            })}
                        </div>
                        <div
                        ...
                        >
                            ...
                            <Btn
                                ...
                                disabled={busy}
                                onClick={() =>
                                    void performSwitch(pendingTarget)
                                }
                            >
          ...
          每个平台用各自的 hint key，互相不显示对方平台的细节。 */}
            <Card>
                ...
                <SettingRow
                    label={t(
                        "settings.advanced.streamingInsertSaveClipboardLabel",
                    )}
                    desc={t(
                        "settings.advanced.streamingInsertSaveClipboardHint",
                    )}
                >
                    <Toggle
                        ...
                        onToggle={(next) => {
                            if (prefs)
                                void updatePrefs({
                                    ...prefs,
                                    streamingInsertSaveClipboard: next,
                                })
                        }}
                    />
                </SettingRow>
            ...
            Foundry 行。 */}
                ...
                这条线在 Win 上不能用，关注点落到下方 Foundry 行。 */}
                        <div style={{ opacity: isWin ? 0.45 : 1 }}>
                            <SettingRow
                                label={t(
                                    "settings.providers.presets.asrLocalQwen3",
                                )}
                                desc={
                                    isMac
                                        ? t("settings.advanced.qwen3Desc")
                                        : t(
                                              "settings.advanced.notSupportedHere",
                                          )
                                }
                            >
                                <div
                        ...
                        {isWin && (
                            <>
                                <SettingRow
                                    label={t(
                                        "settings.providers.presets.asrFoundryLocalWhisper",
                                    )}
                                ...
                                <SettingRow
                                    label={t(
                                        "settings.providers.presets.asrSherpaOnnxLocal",
                                    )}
                                    desc={t("settings.advanced.sherpaDesc")}
                                >
                                    <div
                                        style={{
                                            display: "flex",
            ...
            否则平台本机 Toggle 自身就能 off → 关停，重复 disable 行徒增视觉。 */}
                {isOnAnyLocal &&
                    !(
                        (isMac && isOnLocalQwen3) ||
                        (isWin && (isOnFoundry || isOnSherpaOnnx))
                    ) && (
                        <SettingRow
