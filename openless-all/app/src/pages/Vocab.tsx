	...
	return (
		<>
			<PageHeader
				...
				right={
					<div style={{ display: "flex", gap: 8 }}>
						<Btn icon="refresh" variant="ghost" size="sm" onClick={refreshAll}>
							{t("common.refresh")}
						</Btn>
					</div>
				}
			...
			<Card padding={0}>
				...
				<Collapsible embedded title={t("vocab.corrections.title")} desc={t("vocab.corrections.tip")}>
					<div style={{ display: "grid", gap: 10 }}>
						...
						>
							...
							/>
							<Btn size="sm" variant="primary" onClick={() => void onAddCorrectionRule()}>
								{t("common.add")}
							</Btn>
						</div>
				...
				<Collapsible embedded defaultOpen title={t("vocab.sectionTitle")} desc={t("vocab.tip")}>
					<div style={{ display: "flex", gap: 8 }}>
						<input
							ref={inputRef}
					...
					<div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 8, minHeight: 80 }}>
						...
						{!loading && error && (
							<div style={{ fontSize: 12, color: "var(--ol-err)", lineHeight: 1.6 }}>{t("vocab.loadFailed", { err: error })}</div>
						)}
						{!loading && !error && entries.length === 0 && (
							<div style={{ fontSize: 12, color: "var(--ol-ink-4)" }}>{t("vocab.empty")}</div>
						)}
						{!error &&
							entries.map(e => <VocabChip key={e.id} entry={e} onRemove={() => onRemove(e.id)} onToggle={() => onToggle(e)} />)}
					</div>
				</Collapsible>
			</Card>
			<style>{`
	...
	return (
		<span
			style={{
				...
				fontFamily: "var(--ol-font-mono)",
				transition:
					"background 0.16s var(--ol-motion-quick), opacity 0.18s var(--ol-motion-soft), border-color 0.16s var(--ol-motion-quick)",
		...
		>
			...
			>
				<svg width="8" height="8" viewBox="0 0 8 8">
					<path d="M1 1l6 6M7 1l-6 6" stroke="currentColor" strokeWidth="1.4" />
				</svg>
			</button>
		</span>
	);
