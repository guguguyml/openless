...
import { getHotkeyBindingCodes, getHotkeyBindingLabel, getHotkeyCodeLabel } from "../lib/hotkey";
...
import type { HotkeyBinding, HotkeyMode, HotkeyTrigger, MicrophoneDevice, PasteShortcut } from "../lib/types";
...
export function Settings({ embedded = false, initialSection = "recording" }: SettingsProps) {
	...
	return (
		<>
			{!embedded && <PageHeader kicker={t("settings.kicker")} title={t("settings.title")} />}
...
function RecordingSection() {
	...
	const loadMicrophoneDevices = useCallback(async (signal?: { cancelled: boolean }, options: { showLoading?: boolean } = {}) => {
	...
	const onModeChange = (mode: HotkeyMode) => savePrefs({ ...prefs, hotkey: { ...prefs.hotkey, mode } });
	const onShowCapsuleChange = (showCapsule: boolean) => savePrefs({ ...prefs, showCapsule });
	const onMuteDuringRecordingChange = (muteDuringRecording: boolean) => savePrefs({ ...prefs, muteDuringRecording });
	const onMicrophoneDeviceChange = (microphoneDeviceName: string) => savePrefs({ ...prefs, microphoneDeviceName });
	const onRestoreClipboardChange = (restoreClipboardAfterPaste: boolean) => savePrefs({ ...prefs, restoreClipboardAfterPaste });
	const onPasteShortcutChange = (pasteShortcut: PasteShortcut) => savePrefs({ ...prefs, pasteShortcut });
	...
	const onStartMinimizedChange = (startMinimized: boolean) => savePrefs({ ...prefs, startMinimized });
	const onAutoUpdateCheckChange = (autoUpdateCheck: boolean) => savePrefs({ ...prefs, autoUpdateCheck });
	const onMarketplaceDevLoginChange = (marketplaceDevLogin: string) => savePrefs({ ...prefs, marketplaceDevLogin });
	const onRecordAudioForDebugChange = (recordAudioForDebug: boolean) => savePrefs({ ...prefs, recordAudioForDebug });
	...
	const preferredMicrophoneAvailable = Boolean(
		prefs.microphoneDeviceName && microphoneDevices.some(device => device.name === prefs.microphoneDeviceName),
	);
	const effectiveMicrophoneDeviceName =
		prefs.microphoneDeviceName && (!microphoneDevicesLoaded || preferredMicrophoneAvailable) ? prefs.microphoneDeviceName : "";
	...
	return (
		<>
			<Card>
						...
						{choices.map(([v, l]) => (
							<button
								...
								style={{
									...
									cursor: "default",
									transition:
										"background 0.16s var(--ol-motion-quick), color 0.16s var(--ol-motion-quick), box-shadow 0.18s var(--ol-motion-soft)",
								}}
							>
								{l}
							</button>
				...
				<SettingRow label={t("settings.recording.microphoneLabel")}>
					<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
						...
						{!microphoneDevicesLoaded && <div style={{ fontSize: 11, color: "var(--ol-ink-4)" }}>{t("common.loading")}</div>}
			...
			<Collapsible title={t("settings.recording.insertGroupTitle")}>
				...
				{capability.adapter === "windowsLowLevel" && (
					<SettingRow label={t("settings.recording.allowNonTsfFallbackLabel")}>
						<Toggle on={prefs.allowNonTsfInsertionFallback} onToggle={onAllowNonTsfFallbackChange} />
					</SettingRow>
				)}
			</Collapsible>
			...
			<Collapsible title={t("settings.recording.startupGroupTitle")}>
				...
				{capability.statusHint && (
					<div style={{ marginTop: 6, fontSize: 11.5, color: "var(--ol-ink-4)", lineHeight: 1.5 }}>{capability.statusHint}</div>
				)}
			</Collapsible>
        ...
        Settings 不再提供输入，避免用户改错导致连不上。GitHub OAuth 上线后这里会接登录按钮。 */}
			<Collapsible title={t("settings.recording.marketplaceGroupTitle")}>
				<SettingRow label={t("settings.recording.marketplaceDevLoginLabel")}>
					<input
						type="text"
...
function HotkeyRecorder({ binding, onCommit }: { binding: HotkeyBinding; onCommit: (codes: string[]) => void }) {
	const { t } = useTranslation();
						...
						return (
							<button
								...
								style={{
									...
									background: active ? "rgba(37,99,235,0.08)" : hovered ? "rgba(0,0,0,0.035)" : "#fff",
									boxShadow: active
							...
							>
								<span style={{ minWidth: 0 }}>
									...
									{row.desc && (
										<span style={{ display: "block", fontSize: 11.5, color: "var(--ol-ink-4)", marginTop: 3 }}>{row.desc}</span>
									)}
								</span>
...
function LlmThinkingToggle({ enabled, onToggle }: { enabled: boolean; onToggle: (next: boolean) => void }) {
	...
	return (
		...
		>
			<span style={{ fontSize: 11.5, color: "var(--ol-ink-4)" }}>{t("settings.providers.thinkingModeLabel")}</span>
...
function ProvidersSection() {
	...
	const visibleAsrPresets = ASR_PRESETS.filter(p => p.id !== "foundry-local-whisper" && p.id !== "local-qwen3");
		...
		<>
            ...
            视觉上 label 区出现"字体单独占一行"。下拉自身已经表达了"切换"含义，desc 冗余。 */}
				...
				{codexOAuthSelected ? (
					...
					<>
						<CredentialField
							key={`${committedLlmProvider}:api_key`}
						...
						/>
						<CredentialField
							key={`${committedLlmProvider}:endpoint`}
				...
				<CredentialField
					key={`${committedLlmProvider}:model:${llmModelRevision}`}
            ...
            未激活时不显示提示。 */}
				<SettingRow label={t("settings.providers.providerLabel")}>
					{(() => {
						...
						const anomalousLocal: AsrPresetId | null =
							isLocked && !visibleAsrPresets.some(p => p.id === committedAsrProvider) ? committedAsrProvider : null;
						const anomalousNameKey =
							anomalousLocal === "local-qwen3"
								? "asrLocalQwen3"
				...
				{committedAsrProvider === "volcengine" ? (
				  ...
				  committedAsrProvider ===
						"foundry-local-whisper" ? // 模型管理 UI 唯一入口在 AdvancedSection 里的 <LocalAsr embedded />。 // "在高级中切换或禁用"讲清楚了，body 不再重复 LocalAsrProviderHint。 // 用户已经在用本地 ASR——dropdown 行的 localAsrActiveNotice 已经把
				null : (
					<>
						<CredentialField
						...
						/>
						<CredentialField
							key={`${committedAsrProvider}:endpoint`}
							label={t("settings.providers.baseUrlLabel")}
						...
						/>
						<CredentialField
							key={`${committedAsrProvider}:model:${asrModelRevision}`}
							label={t("settings.providers.modelLabel")}
							account="asr.model"
		...
		<SettingRow label={t("settings.providers.toolsLabel")}>
			<div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", maxWidth: 420 }}>
				<div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
					<button onClick={validate} style={miniBtnStyle} disabled={status === "loading"}>
						{t("settings.providers.validate")}
					</button>
					<button onClick={loadModels} style={miniBtnStyle} disabled={status === "loading"}>
				...
				{message && (
					<span
						style={{
							fontSize: 11,
							color: status === "error" ? "var(--ol-warn)" : status === "empty" ? "var(--ol-ink-4)" : "var(--ol-ok)",
							lineHeight: 1.4,
						}}
					>
						{message}
	...
	return (
		<SettingRow label={label}>
			<div style={{ display: "flex", gap: 6, alignItems: "center", width: "100%", maxWidth: 420 }}>
				...
				<button onClick={onCopy} title={t("common.copy")} style={iconBtnStyle} disabled={!value || disabled}>
...
const hotkeyRecorderButtonStyle: CSSProperties = {
	...
	cursor: "default",
	transition:
		"background 0.16s var(--ol-motion-quick), border-color 0.16s var(--ol-motion-quick), color 0.16s var(--ol-motion-quick)",
};
...
function LocalAsrProviderHint({
	...
	const hintKey =
		provider === "foundry-local-whisper" ? "settings.providers.foundryLocalAsrHint" : "settings.providers.localAsrHint";

	if (loading) {
		return <div style={{ padding: "12px 0", fontSize: 12.5, color: "var(--ol-ink-4)" }}>{t("common.loading")}</div>;
	}
	...
	if (provider === "foundry-local-whisper") {
		return (
			<div style={{ padding: "8px 0 4px", display: "flex", flexDirection: "column", gap: 12 }}>
				<div style={{ fontSize: 12.5, color: "var(--ol-ink-3)", lineHeight: 1.6 }}>{t(hintKey)}</div>
				<div>
					...
					{downloaded.map(m => (
						...
						>
							<div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
								...
								<span style={{ fontSize: 11, color: "var(--ol-ink-4)" }}>{formatBytes(m.downloadedBytes)}</span>
							</div>
							<Btn variant="ghost" size="sm" disabled={deletingId === m.id} onClick={() => void handleDelete(m.id)}>
